...
namespace RasHack.GapOverlap.Main.Result.Fixations.CSV
{
    public class SingleGazeCSV : DumpToCSV
    {
        ...
        {
            return $"{Quote($"{classifier} gaze after s")},{Quote($"{classifier} gaze duration s")},{Quote($"{classifier} best fixation s")},{Quote($"{classifier} p25 distance in degrees")},{Quote($"{classifier} p50 distance in degrees")},{Quote($"{classifier} p95 distance in degrees")}";
        }
        ...
        public string ToCSV(SingleGaze gaze)
        {
            ...
            return $"{Quote(after)},{Quote(duration)},{Quote(fixation)},{Quote(p25Distance)},{Quote(p50Distance)},{Quote(p95Distance)}";
        }

        public string CSVHeaderAveraged(string classifier)
        {
            return $"{Quote($"{classifier} gaze after s on average")},{Quote($"{classifier} average gaze duration s")},{Quote($"{classifier} average best fixation s")},{Quote($"{classifier} p25 distance in degrees")},{Quote($"{classifier} p50 distance in degrees")},{Quote($"{classifier} p95 distance in degrees")}";
        }
